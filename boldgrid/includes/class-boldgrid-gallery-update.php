 ...
 *
 * @package Boldgrid_Gallery_Update
...
class Boldgrid_Gallery_Update {
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			...
			$is_plugin_information = ( 'plugin-install.php' === $pagenow && isset( $_GET['plugin'] ) &&
				 'boldgrid-gallery' === $_GET['plugin'] && isset( $_GET['tab'] ) &&
	...
	public function custom_plugins_transient_update( $transient ) {
		...
		if ( is_multisite() ) {
			$version_data = get_site_transient( 'boldgrid_gallery_version_data' );
		} else {
		...
		if ( $have_configs && ( false === $version_data || ( $is_force_check && $is_data_old ) ) ) {
			...
			$params_array['key'] = 'gallery-wc-canvas';
			$params_array['channel'] = $channel;
			$params_array['installed_' . $params_array['key'] . '_version'] = $plugin_data['Version'];
			...
			if ( false === empty( $version_data ) && 200 === $version_data->status &&
				...
				if ( is_multisite() ) {
					delete_site_transient( 'boldgrid_gallery_version_data' );
					set_site_transient( 'boldgrid_gallery_version_data', $version_data,
						8 * HOUR_IN_SECONDS );
				} else {
					delete_transient( 'boldgrid_gallery_version_data' );
					set_transient( 'boldgrid_gallery_version_data', $version_data,
						8 * HOUR_IN_SECONDS );
				}
			} else {
		...
		if ( 'plugin-install.php' === $pagenow && isset( $_GET['plugin'] ) &&
			...
			$transient->download_link = $configs['asset_server'] .
				 ...
				 '&installed_wc_gallery_version=' . $plugin_data['Version'] .
				 '&installed_wp_version=' . $wp_version;

			if ( false === empty( $version_data->result->data->compatibility ) && ( $compatibility = json_decode(
				$version_data->result->data->compatibility, true ) ) !== null ) {
			...
			$transient->plugin_name = 'boldgrid-gallery.php';
			$transient->slug = 'boldgrid-gallery';
			...
			$obj = new stdClass();
			$obj->slug = 'boldgrid-gallery';
			$obj->plugin = 'boldgrid-gallery/wc-gallery.php';
			...
			$obj->package = $configs['asset_server'] . $configs['ajax_calls']['get_asset'] . '?id=' .
				 $version_data->result->data->asset_id . '&installed_wc_gallery_version=' .
